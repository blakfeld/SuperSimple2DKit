...
public class NewPlayer : PhysicsObject {
    ...
    [SerializeField]
    private AudioSource flameParticlesAudioSource;
    ...
    [SerializeField]
    private float fallForgiveness = .2f; //How long the player can fall from a ledge and still jump
    ...
    private float
        launch; //The float added to x and y moveSpeed. This is set with hurtLaunchPower, and is always brought back to zero

    [SerializeField]
    private float
        launchRecovery; //How slow should recovering from the launch be? (Higher the number, the longer the launch will last)
    ...
    protected void ComputeVelocity() {
        ...
        if (
            ...
            animator.GetBool("grounded") == true &&
            !jumping
        ) {
        ...
        animator
            .SetInteger("attackDirectionY",
                (int) Input.GetAxis("VerticalDirection"));
        animator
            .SetInteger("moveDirection",
                (int) Input.GetAxis("HorizontalDirection"));
        animator
            .SetBool("hasChair",
                GameManager.Instance.inventory.ContainsKey("chair"));
    ...
    private void HurtEffect() {
        ...
        GameManager
            .Instance
            .audioSource
            .PlayOneShot(hurtSounds[whichHurtSound]);
    ...
    public IEnumerator Die() {
        if (!frozen) {
            ...
            GameManager.Instance.hud.loadSceneName =
    ...
    public void Jump(float jumpMultiplier) {
        if (velocity.y != jumpPower) {
            velocity.y =
                jumpPower *
                jumpMultiplier; //The jumpMultiplier allows us to use the Jump function to also launch the player from bounce platforms
            PlayJumpSound();
            PlayStepSound();
            JumpEffect();
            jumping = true;
    ...
    public void PlayStepSound() {
        ...
        audioSource
            .PlayOneShot(stepSound,
                Mathf.Abs(Input.GetAxis("Horizontal") / 10));
    }
    ...
    public void ActivatePound() {
        ...
        if (!pounding) {
            ...
            GameManager
                .Instance
                .audioSource
                .PlayOneShot(poundActivationSounds[Random
                    .Range(0, poundActivationSounds.Length)]);
            pounding = true;
